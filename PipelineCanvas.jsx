...
// BUG FIX #7 (SQL regex) : La regex originale utilisait [\s\S]*? (non-greedy)
// ce qui causait un arrêt prématuré si un ';' apparaissait dans les contraintes.
// SOLUTION : parser le DDL caractère par caractère en comptant les parenthèses
// pour trouver le bloc complet de chaque CREATE TABLE, sans regex fragile.
function parseSqlTables(sqlCode) {
  const tables = [];
  if (!sqlCode) return tables;

  const upperSql = sqlCode.toUpperCase();
  let searchFrom = 0;

  while (true) {
    // Trouver le prochain CREATE TABLE
    const createIdx = upperSql.indexOf('CREATE TABLE', searchFrom);
    if (createIdx === -1) break;

    // Trouver la parenthèse ouvrante
    const parenOpen = sqlCode.indexOf('(', createIdx);
    if (parenOpen === -1) break;

    // Extraire le nom de la table entre CREATE TABLE et (
    const tableNameRaw = sqlCode.substring(createIdx + 12, parenOpen).trim();
    // Gérer IF NOT EXISTS
    ...
    // FIX : compter les parenthèses pour trouver le bloc complet
    // (la regex [\s\S]*? échouait si un ';' était dans les contraintes)
    let depth = 0;
    let i = parenOpen;
    let parenClose = -1;
    for (; i < sqlCode.length; i++) {
      if (sqlCode[i] === '(') depth++;
      else if (sqlCode[i] === ')') {
        depth--;
    ...
    const content = sqlCode.substring(parenOpen + 1, parenClose);

    // Parser les colonnes (exclure les contraintes de table)
    const lines = content.split('\n')
      .map(c => c.trim().replace(/,$/, ''))
      .filter(c => {
        if (!c) return false;
        const upper = c.toUpperCase();
        // Exclure les contraintes de table (FOREIGN KEY, PRIMARY KEY, CONSTRAINT, KEY, INDEX)
        ...
        if (upper.startsWith('KEY ') || upper.startsWith('KEY\t')) return false;
        if (upper.startsWith('INDEX ')) return false;
        if (upper.startsWith('UNIQUE KEY')) return false;
        return true;
      });

    const isFact = tableName.toLowerCase().includes('fact') || tableName.toLowerCase().includes('fait');
    tables.push({ id: tableName, cols: lines, isFact });
...
function FlowArea({ sqlCode, etlCode, pipelineState }) {
  ...
  // BUG FIX #6 : getS était une fonction inline dans useEffect, ce qui la recréait
  // à chaque render. Maintenant stabilisée avec useCallback pour éviter
  // les dépendances instables du useEffect principal.
  const getS = useCallback((id) => {
    const logs = pipelineState?.logs || [];
    const logText = logs.map(l => l.msg).join(' ');
    const stageStatus = (sid) => pipelineState?.stages?.find(x => x.id === sid)?.status || 'idle';
    ...
    return 'idle';
  }, [sqlCode, etlCode, pipelineState, bottomProgress]); // Dépendances stables et explicites
  ...
  useEffect(() => {
    if (viewMode === 'mcd') {
      ...
      // BUG FIX #7 : utiliser parseSqlTables() au lieu de la regex fragile
      const allTables = parseSqlTables(sqlCode);
      const mcdNodes = [];
      const mcdEdges = [];
      const dims = allTables.filter(t => !t.isFact);
      const fact = allTables.find(t => t.isFact) || null;

      if (fact) {
        mcdNodes.push({
          id: fact.id, type: 'table', position: { x: 500, y: 350 },
          data: { label: fact.id, cols: fact.cols, isFact: true }
        });
        const radius = 350;
        ...
        dims.forEach((d, i) => {
          ...
          mcdNodes.push({
            id: d.id, type: 'table', position: { x, y },
            data: { label: d.id, cols: d.cols, isFact: false }
          });
          mcdEdges.push({
  ...
  // BUG FIX #6 : Les dépendances originales incluaient onNodesChange, onEdgesChange
  // et fitView — toutes recréées à chaque render → boucle infinie de re-renders.
  // SOLUTION : exclure ces setters stables des dépendances (eslint-disable commenté
  // pour documenter le choix intentionnel). getS est maintenant stabilisé via useCallback.
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [viewMode, sqlCode, etlCode, pipelineState, bottomProgress, getS]);
            ...
            <div className="bg-rose-500/10 backdrop-blur-md border border-rose-500/30 p-4 rounded-xl flex items-start gap-4 shadow-[0_10px_40px_rgba(244,63,94,0.15)]">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-rose-200/70 leading-relaxed mb-4">
                  {pipelineState?.error || "Le pipeline a rencontré une défaillance inattendue."}
                </p>
                {/* BUG FIX #11 (original) : window.location.reload() effaçait toute
                    la session MemorySaver sans avertissement. Remplacé par un rechargement
                    avec confirmation pour prévenir la perte de données. */}
                <button
                  onClick={() => {
                    if (window.confirm("⚠️ Redémarrer effacera la session en cours (données non sauvegardées perdues). Continuer ?")) {
